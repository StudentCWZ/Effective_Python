...
"""
    ...
    7. 总结：
        - 在 Python 3 中，bytes 是一种包含 8 位值的序列，str 是一种包含 Unicode 字符的序列。开发者不能以 > 或 + 等操作符来混同操作
          bytes 和 str 实例。
        - 在 Python 2 中，str 是一种包含 8 位值的序列，unicode 是一种包含 Unicode 字符的序列。如果 str 只含有 7 位 ASCII 字符，那么
          可以通过相关操作来同时使用 str 和 unicode
        - 在对输入的数据进行操作之前，适用辅助函数来保证字符的类型与开发者的期望相符。
        - 从文件中读取二进制数据，或向其写入二进制数据时，总应该以 'rb' 或者 'wb' 等二进制模式来启动文件。
"""
